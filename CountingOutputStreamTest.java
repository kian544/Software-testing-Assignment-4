...
import java.lang.reflect.Field; //added since CountingOutputStream is private
...
public class CountingOutputStreamTest {
    ...
    // Added to test boundary condition where count reaches Integer.MAX_VALUE
    @Test
    public void testGetCountBoundary() throws Exception {
        final ByteArrayOutputStream baos = new ByteArrayOutputStream();
        try (final CountingOutputStream cos = new CountingOutputStream(baos)) {
            setPrivateCount(cos, Integer.MAX_VALUE);
            assertEquals("CountingOutputStream.getCount() at boundary", Integer.MAX_VALUE, cos.getCount());
        }
    }

    // Added to verify ArithmeticException when count exceeds Integer.MAX_VALUE
    @Test(expected = ArithmeticException.class)
    public void testGetCountExceedIntMax() throws Exception {
        final ByteArrayOutputStream baos = new ByteArrayOutputStream();
        try (final CountingOutputStream cos = new CountingOutputStream(baos)) {
            setPrivateCount(cos, (long) Integer.MAX_VALUE + 1);
            cos.getCount(); // Should throw an exception
        }
    }

    // Added to check resetCount works correctly at Integer.MAX_VALUE
    @Test
    public void testResetCountBoundary() throws Exception {
        final ByteArrayOutputStream baos = new ByteArrayOutputStream();
        try (final CountingOutputStream cos = new CountingOutputStream(baos)) {
            setPrivateCount(cos, Integer.MAX_VALUE);
            assertEquals("CountingOutputStream.resetCount() at boundary", Integer.MAX_VALUE, cos.resetCount());
            assertEquals("CountingOutputStream.getCount() after reset", 0, cos.getCount());
        }
    }

    // Added to verify ArithmeticException when resetCount is called with a count exceeding the max value
    @Test(expected = ArithmeticException.class)
    public void testResetCountExceedIntMax() throws Exception {
        final ByteArrayOutputStream baos = new ByteArrayOutputStream();
        try (final CountingOutputStream cos = new CountingOutputStream(baos)) {
            setPrivateCount(cos, (long) Integer.MAX_VALUE + 1);
            cos.resetCount(); // Should throw an exception
        }
    }

    // Uses "java reflection" to modify the private count var in main file
    private void setPrivateCount(CountingOutputStream cos, long value) throws Exception {
        Field countField = CountingOutputStream.class.getDeclaredField("count");
        countField.setAccessible(true);
        countField.setLong(cos, value);
    }
